...
void scene12::RenderSkybox()
{
	...
	RenderTextOnScreen(meshList[GEO_TEXT], "FPS: " + to_string(nbFrames), Color(0, 1, 0), 4, 0, 0);
}
void scene12::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void scene12::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void scene12::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST);
